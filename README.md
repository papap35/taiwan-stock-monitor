...
### ⏰ 自動 AI 簡報排程
- 每個交易日 08:45 盤前分析 + 13:35 盤後總結，自動推播至 LINE
- 整合即時大盤、三大法人籌碼、國際市場資料生成 AI 分析
- 設定頁可獨立開啟/關閉盤前與盤後，支援「立即觸發」測試
...
### 🔔 瀏覽器推播通知
- 警報觸發時同步顯示桌面系統通知，不需切換至 LINE
- 設定頁一鍵開啟，瀏覽器 Notification API 授權後即生效（前景通知）

### 📅 重要事件行事曆
- 除息 / 除權 / 財報申報截止日（資料來源：TWSE OpenAPI）
- 依日期分組顯示，3 天內事件高亮提醒
- 可篩選事件類型、設定天數範圍（7/14/30/60 天）
- 自選股行顯示最近事件提醒標籤（💰 除息 / 📈 除權 / 📋 財報）

### 📢 個股 K 線事件標記
- K 線圖自動標記除息（💰）/ 除權（📈）/ 財報（📋）日，對應 K 線顯示彩色箭頭或圓點
- 圖表下方列出該股全部事件清單，依日期降冪排列
...
- **族群快速代入**：12 個預設概念股族群（AI伺服器、重電、航運、金融、ETF…），一鍵填入或合併至代號輸入框
- 熱門股票頁支援族群篩選，快速查看特定概念股的漲跌排行

### 📐 多股同列比較（疊圖）
- 自選股勾選最多 5 檔，以「期初 = 0%」漲跌幅 % 疊圖比較相對強弱
- 區間切換 1M / 3M / 6M / 1Y，圖例可點擊切換顯示/隱藏個別股票
