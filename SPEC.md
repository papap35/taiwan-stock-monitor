...
**已實作**：
- 靜態資料 `frontend/src/utils/industryGroups.js`：12 個族群（AI伺服器/CoWoS、重電/電網、航運、半導體設備、金融、ETF主要、面板、電動車/電池、生技/醫療、消費電子/代工、網路/雲端、建設/不動產），含 4 個結構驗證測試
- 條件選股掃描器：代號輸入框上方新增族群下拉選單，選擇族群後顯示「取代輸入」/「合併輸入」兩個按鈕，分別覆蓋或附加代號至輸入框
- 熱門股票頁：篩選列新增「全部族群 / 族群名稱」下拉選單，選擇後前端篩選 `hotStocks` 只顯示該族群的代號，篩選中以藍色外框高亮顯示
...
**已實作**：
- `frontend/src/utils/etfData.js`：6 個常見市值型/高股息 ETF 靜態資料（0050、0056、006208、00878、00919、00929），含追蹤指數、配息頻率、前 10 大成分股權重、更新日期
- `isKnownEtf(code)` / `getEtfInfo(code)` 輔助函式，9 個結構驗證測試
- StockChart「基本面」tab 新增 `EtfInfoCard`：追蹤指數 + 配息頻率 KPI 卡、前 10 大成分股權重橫條圖
- 殖利率沿用既有 `valuation`（BWIBBU_d）資料，無需額外 API；若個股無 BWIBBU_d 資料但屬已知 ETF，仍會單獨顯示 ETF 卡片
